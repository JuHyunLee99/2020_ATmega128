 ...
 *  Author: 이주현
 */ 
//0.5초이상 2초미만 100msec시간지연, 2초이상 50msec 시간지연
#include <avr/io.h>
...
#define PRESS_THRESHOLD1			16
#define PRESS_THRESHOLD2			66
...
unsigned char BtnInput_Press(unsigned char *pressed)
{
	...
	*pressed = 0;
	 ...
	 {
		 press_cnt++;			//누른 시간 증가
		 if((press_cnt > PRESS_THRESHOLD1) & (press_cnt < PRESS_THRESHOLD2 ))
		 { 
		...
		else if(press_cnt > PRESS_THRESHOLD2)
		{
			if(sw != 0)		//안누르고 있을 경우 제거
			{
				*pressed = 1;
